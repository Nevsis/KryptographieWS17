...
def MixCol(A):
  ...
  for i in range (4):
    for j in range (4):
      for n in range (4):
        erg = F256Add(erg, F256Mul(mulMatrix[i][n], A[n][j]))
      B[i][j] = erg
      erg = 0;
  return B
...
def AES(P,K):
  ...
  # first step only add key cause can reverse operations before first key is added
  F = AddKey(P, K[0])
  for i in range(1,10):
    F = AESRound(F, K[i])

  # last step no MixCol for whateever reason
  F = AddKey(ShiftRow(SubByte(F)), K[10])
  return F
